...
//
// The -f flag lists uncovered functions by name instead of showing code blocks.
// The -t N flag lists the top N functions with the lowest coverage percentage.
// The -s flag outputs in a sortable/diffable format suitable for comparing different coverage runs.
package main
...
var (
	...
	showFunctions = flag.Bool("f", false, "list uncovered functions by name")
	topN          = flag.Int("t", 0, "list top N functions with lowest coverage percentage")
	sortable      = flag.Bool("s", false, "output in a sortable/diffable format")
)

func usage() {
	fmt.Fprintf(os.Stderr, "usage: uncover [-a] [-l] [-f] [-t N] [-s] c.out\n")
	os.Exit(2)
}
...
func uncover(profile string) ([]byte, error) {
	...
	// If function reporting is enabled
	if *showFunctions || *topN > 0 {
		var allFunctions []FunctionCoverage

		// Process each file for function coverage
		for _, profile := range profiles {
			...
			functions, err := findFunctions(file, src)
			if err != nil {
				return nil, err
			}

			// Calculate coverage for functions
			functions = calculateFunctionCoverage(functions, profile)
			allFunctions = append(allFunctions, functions...)
		}

		// Generate reports based on flags
		if *showFunctions {
			buf.WriteString(listUncoveredFunctions(allFunctions))
		}

		if *topN > 0 {
			buf.WriteString(listTopUncoveredFunctions(allFunctions, *topN))
		}
	} else {
		// Standard uncovered code blocks report
		for _, profile := range profiles {
			fn := profile.FileName
			file, err := findFile(dirs, fn)
			if err != nil {
...
func uncoverFile(buf *bytes.Buffer, file string, src []byte, bounds []Boundary) error {
	...
	// For sortable output, collect all uncovered blocks
	if *sortable {
		var uncoveredBlocks []struct {
			File  string
			Addr  string
			Code  []byte
		}
	...
	SortableSnippets:
		for i := 0; i+1 < len(bounds); i += 2 {
			...
			uncoveredBlocks = append(uncoveredBlocks, struct {
				File  string
				Addr  string
				Code  []byte
			}{
		...
		// Sort blocks by file path and line number for consistent output
		sort.Slice(uncoveredBlocks, func(i, j int) bool {
			if uncoveredBlocks[i].File != uncoveredBlocks[j].File {
				return uncoveredBlocks[i].File < uncoveredBlocks[j].File
			}
			return uncoveredBlocks[i].Addr < uncoveredBlocks[j].Addr
		})

		// Write sorted blocks
		buf.WriteString("# Uncovered code blocks\n")
		for _, block := range uncoveredBlocks {
			fmt.Fprintf(buf, "%s:%s\n", block.File, block.Addr)
			buf.Write(block.Code)
			if len(block.Code) > 0 && block.Code[len(block.Code)-1] != '\n' {
				buf.WriteByte('\n')
			}
		}
